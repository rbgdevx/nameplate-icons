--- AceConfigDialog-4.0 generates AceGUI-4.0 based windows based on option tables.
-- @class file
-- @name AceConfigDialog-4.0
-- @release $Id: AceConfigDialog-4.0.lua 1296 2022-11-04 18:50:10Z nevcairiel $

local LibStub = LibStub
local gui = LibStub("AceGUI-4.0")
local reg = LibStub("AceConfigRegistry-4.0")

local MAJOR, MINOR = "AceConfigDialog-4.0", 86
...
AceConfigDialog.tooltip = AceConfigDialog.tooltip
  or CreateFrame("GameTooltip", "AceConfigDialog40Tooltip", UIParent, "GameTooltipTemplate")
    ...
    if grouptype == "tab" then
      ...
      for i = 1, #tabs do
        ...
        if not entry.disabled then
          tab:SelectTab(
            (GroupExists(appName, options, path, status.groups.selected) and status.groups.selected) or entry.value
          )
          break
        end
      end
    ...
    elseif grouptype == "select" then
      ...
      if firstgroup then
        selectGroup:SetGroup(
          (GroupExists(appName, options, path, status.groups.selected) and status.groups.selected) or firstgroup
        )
      end
